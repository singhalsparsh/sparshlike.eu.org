...
export function FullscreenPlayer() {
  ...
  const handleTiltMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setTiltX(((y - centerY) / centerY) * -3);
    setTiltY(((x - centerX) / centerX) * 3);
  }, []);
  ...
  const videoLoopStart = useMemo(() => {
    ...
    if (duration < 90) return Math.floor(duration * 0.15); // short videos
    // Deterministic hash from videoId for consistent segments per track
  ...
  const videoSrc = showVideo && displayVideoId
    ? `https://www.youtube.com/embed/${displayVideoId}?autoplay=1&loop=1&controls=0&showinfo=0&rel=0&iv_load_policy=3&playsinline=1&mute=1&start=${videoLoopStart}&end=${videoLoopEnd}&playlist=${displayVideoId}`
    : '';
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex flex-col animate-in fade-in duration-300">
      ...
      {/* Main content with 3D perspective tilt */}
      <div
        ref={contentRef}
        onMouseMove={handleTiltMove}
        onMouseEnter={handleTiltEnter}
        onMouseLeave={handleTiltLeave}
        className="relative z-10 flex flex-col h-full w-full max-w-6xl mx-auto px-4 sm:px-8 py-4 sm:py-6"
        style={{
          transform: isTiltHovered
            ? `perspective(1200px) rotateX(${tiltX}deg) rotateY(${tiltY}deg)`
            : undefined,
          transition: isTiltHovered ? 'transform 0.1s ease-out' : 'transform 0.5s ease-out',
        }}
      >
        {/* Top bar */}
        ...
        <div className="flex-1 flex items-center justify-center overflow-hidden">
          <div className="flex flex-col lg:flex-row items-center lg:items-start gap-8 lg:gap-16 w-full max-w-5xl">
            {/* Left: Album art */}
            <div className="relative shrink-0">
              <div className={cn(
                'relative w-48 h-48 sm:w-64 sm:h-64 md:w-72 md:h-72 rounded-full overflow-hidden',
            ...
            <div className="flex flex-col gap-5 w-full max-w-lg">
              <div>
                ...
                <p className="text-sm sm:text-base text-white/50 mt-1 font-medium tracking-wide">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  onClick={togglePlay}
                  className="flex items-center justify-center w-12 h-12 rounded-full bg-white text-black hover:scale-105 active:scale-95 transition-all shadow-[0_8px_24px_rgba(0,0,0,0.3)]"
              ...
              <div
                ...
                className="relative flex-1 min-h-0 max-h-[200px] sm:max-h-[260px] overflow-y-auto rounded-xl bg-white/[0.03] border border-white/[0.06] p-4"
